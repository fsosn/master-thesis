...
\end{table}
Zauważyć można wysoką skuteczność tego ataku. Model Mistral-7B-Instruct-v0.3 zwrócił szkodliwą odpowiedź dla każdego testowanego przypadku. Z kolei model Llama-3.1-8B-Instruct był niemal dwukrotnie bardziej od niego odporny, lecz otrzymany \gls{ASR} wciąż był na bardzo wysokim poziomie, ponieważ aż 52\%. Dowodzi to, że oba modele są podstawowo podatne na ten atak automatyczny i wbudowane zabezpieczenia nie są wystarczające do ochrony przed dynamicznie dostosowywanymi poleceniami wejściowymi przez inny model atakujący.
...
\begin{table}[!ht]
    ...
    \begin{tabular}{l|c|c|c|c}
        ...
        Mistral-7B-Instruct-v0.3 & 49                           & 96  & 58  & 100  \\\hline
    \end{tabular}
    \caption{\label{tab:comparison_tab}Skuteczność badanych metod ataku bez zastosowania mechanizmów obronnych}
\end{table}
Najwyższą skuteczność ataku na model Llama-3.1-8B-Instruct otrzymano przy użyciu metody DAN i wyniosła ona 55\%. Podobne \gls{ASR} otrzymano przeprowadzając atak \gls{PAIR}, ponieważ rezultat różnił się jedyne 3 punkty procentowe. Znacznie gorzej wypadł na tle innych atak GCG, który przełamał wbudowaną ochronę tylko w 13 przypadkach, czyli o odpowiednio 42 i 39 mniej niż przy użyciu \gls{DAN} oraz \gls{PAIR}. Nieduży wzrost poziomu \gls{ASR} dla ataku GCG odnotowano również w przypadku modelu Mistral-7B-Instruct-v0.3. Natomiast najskuteczniejszym wobec niego atakiem była metoda \gls{PAIR}, która osiągnęła maksymalny wynik. Można dojść do wniosku, że metody oparte na dostosowywaniu promptów są najskuteczniejszymi metodami naruszania wbudowanych zabezpieczeń modelu. Osiągnęły znacznie lepszy rezultat niż atak optymalizujący sufiksy dodawane do poleceń wejściowych. Można również dojść do wniosku, że oba modele wymagają dodatkowych mechanizmów obronnych, by zmniejszyć potencjalne powodzenie ataków.
\begin{figure}[!ht]
    \centering
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[fill=blue!70!black] coordinates {(Podstawowo,6) (DAN,55) (GCG,13) (PAIR,52)};
            \addplot[fill=red!70!black] coordinates {(Podstawowo,49) (DAN,96) (GCG,58) (PAIR,100)};
